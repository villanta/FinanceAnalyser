...
public class BarclaysTransactionReader implements TransactionReader {

	private static final Logger LOG = LogManager.getLogger(BarclaysTransactionReader.class);

	@Override
	public Transaction parseTransaction(String transaction) {
		String[] transactionFields = transaction.split(",");

		LocalDate date = parseDate(transactionFields[1]);
		TransactionType type = parseType(transactionFields[4], transactionFields[5]);
		int amount = parseAmount(transactionFields[3], type);
		String name = parseName(transactionFields[5]);
	...
	private int parseAmount(String moneyFlow, TransactionType tt) {
